...
//    - V1.1 
//        - Added stiffeners to better carry the load of the pulley
...
//    - V1.2 
//        - Enlarged the walls around the rods to prevent cracking
//        - Reduced lower stiffeners for better end stop integration
  ...
  difference() {
    union() {
      ...
      translate([0, 7.5,   0]) rotate([90 - tilt, 0, 0]) bearing_mount();
   	  translate([0, 15, -h/2]) stifener(50, 2, 2, 45);
	  translate([0, 15,  h/2]) rotate([0, 180, 0]) stifener(50, 7, 2, 45);
	  translate([0, 15,    0]) cube([6, 2, h], center=true);
	  translate([0, 15,    0]) rotate([90, 0, 0]) cylinder(r=8, h=2, center=true, $fn=24);
    }
